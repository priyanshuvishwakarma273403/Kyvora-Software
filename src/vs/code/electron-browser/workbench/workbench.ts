...
(async function () {
	...
	function showDefaultSplash(configuration: INativeWindowConfiguration) {
		...
		style.textContent = `
			...
			@keyframes kyvora-loading {
				0% { left: -60px; }
				100% { left: 140px; }
			}
			@keyframes kyvora-float {
		...
		// Always create splash element to display premium Kyvora loading experience
		const splash = document.createElement('div');
		splash.id = 'monaco-parts-splash';
		splash.className = baseTheme ?? 'vs-dark';

		// Create centered premium Kyvora brand logo and animated loader
		...
		logoContainer.style.animation = 'kyvora-float 3s ease-in-out infinite';
		...
		textContainer.style.fontFamily = "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
		textContainer.style.fontSize = '26px';
		textContainer.style.fontWeight = '700';
		textContainer.style.color = '#e0d8f0';
		textContainer.style.letterSpacing = '3px';
		textContainer.style.textShadow = '0 0 10px rgba(169, 141, 232, 0.5)';
		textContainer.textContent = 'KYVORA';

		// Create loader container
		const loaderContainer = document.createElement('div');
		loaderContainer.style.marginTop = '15px';
		loaderContainer.style.width = '140px';
		loaderContainer.style.height = '3px';
		loaderContainer.style.background = 'rgba(169, 141, 232, 0.15)';
		loaderContainer.style.borderRadius = '2px';
		...
		loaderBar.style.width = '60px';
		loaderBar.style.background = 'linear-gradient(90deg, #a98de8, #c8b4f0)';
		loaderBar.style.borderRadius = '2px';
		loaderBar.style.animation = 'kyvora-loading 1.6s infinite ease-in-out';
		...
		splash.appendChild(centerContainer);
